...
use crate::group::perm_group::PermGroup;
...
impl Perm {
    pub(crate) fn from_images_unchecked(n: usize, images: Vec<usize>) -> Self {
        Self { n, images }
    }

    pub(crate) fn images_ref(&self) -> &[usize] {
        &self.images
    }
}
...
impl PermSubgroup {
    pub(crate) fn from_generated(n: usize, gens: Vec<Perm>, max_size: usize) -> PyResult<Self> {
...
impl Perm {
    ...
    /// Support `==` and `!=`.
    pub fn __richcmp__(&self, other: &Perm, op: pyo3::basic::CompareOp) -> PyResult<bool> {
        match op {
            pyo3::basic::CompareOp::Eq => Ok(self.n == other.n && self.images == other.images),
            pyo3::basic::CompareOp::Ne => Ok(self.n != other.n || self.images != other.images),
            _ => Err(PyValueError::new_err("Only == and != supported")),
        }
    }

    /// Support using Perm as dict keys / set members.
    pub fn __hash__(&self) -> u64 {
        let mut h: u64 = 17;
        h = h.wrapping_mul(31).wrapping_add(self.n as u64);
...
impl PermSubgroup {
    ...
    /// Support `==` and `!=`.
    pub fn __richcmp__(&self, other: &PermSubgroup, op: pyo3::basic::CompareOp) -> PyResult<bool> {
        match op {
            pyo3::basic::CompareOp::Eq => Ok(self.n == other.n && self.elements == other.elements),
            pyo3::basic::CompareOp::Ne => Ok(self.n != other.n || self.elements != other.elements),
            _ => Err(PyValueError::new_err("Only == and != supported")),
        }
    }

    /// Support using PermSubgroup as dict keys / set members.
    pub fn __hash__(&self) -> u64 {
        let mut h: u64 = 17;
        h = h.wrapping_mul(31).wrapping_add(self.n as u64);
        for g in &self.elements {
            h = h.wrapping_mul(31).wrapping_add(g.__hash__());
        }
        h
    }
...
impl Sn {
    ...
    pub fn elements(&self, max_size: Option<usize>) -> PyResult<Vec<Perm>> {
        let max = max_size.unwrap_or_else(|| factorial(self.n));
    ...
    pub fn generated(&self, gens: Vec<Perm>) -> PyResult<PermSubgroup> {
        self.generated_with_limit(gens, factorial(self.n))
    }
    ...
    /// Return a non-enumerating `PermGroup` generated by `gens`.
    pub fn generated_group(&self, gens: Vec<Perm>) -> PyResult<PermGroup> {
        PermGroup::from_generators(self.n, gens)
    }
    ...
    #[pyo3(signature = (perm, max_size = None))]
