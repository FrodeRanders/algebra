...
impl Fq {
    ...
    pub fn multiplicative_group_perms(&self) -> PyResult<Vec<Perm>> {
        self.multiplicative_group_perms_with_limit(4096)
    }

    /// Return the permutation representation of the multiplicative group acting by `x -> a*x`.
    pub fn multiplicative_group_perms_with_limit(&self, max_size: u64) -> PyResult<Vec<Perm>> {
        self.elements(Some(max_size))?
            .into_iter()
            .filter(|a| !a.coeffs.is_empty())
            .map(|a| self.mul_perm_with_limit(&a, max_size))
            .collect()
    }

    /// Return the size of the subgroup generated by the multiplicative action `x -> a*x`.
    /// Uses the default enumeration bound `max_size = 4096`.
    pub fn multiplicative_action_subgroup_size(&self) -> PyResult<usize> {
        self.multiplicative_action_subgroup_size_with_limit(4096)
    }

    /// Return the size of the subgroup generated by the multiplicative action `x -> a*x`.
    pub fn multiplicative_action_subgroup_size_with_limit(&self, max_size: u64) -> PyResult<usize> {
        Sn::new(self.size() as usize)?.subgroup_size(self.multiplicative_group_perms_with_limit(max_size)?)
    }

    /// Return the orbit decomposition under the multiplicative action `x -> a*x`.
    /// Uses the default enumeration bound `max_size = 4096`.
    pub fn multiplicative_action_orbits(&self) -> PyResult<Vec<Vec<usize>>> {
        self.multiplicative_action_orbits_with_limit(4096)
    }

    /// Return the orbit decomposition under the multiplicative action `x -> a*x`.
    pub fn multiplicative_action_orbits_with_limit(&self, max_size: u64) -> PyResult<Vec<Vec<usize>>> {
        Sn::new(self.size() as usize)?.orbits(self.multiplicative_group_perms_with_limit(max_size)?)
    }
    ...
    pub fn multiplicative_action_stabilizer(&self, point: usize) -> PyResult<Vec<Perm>> {
        self.multiplicative_action_stabilizer_with_limit(point, 4096)
    }

    /// Return the stabilizer subgroup of `point` under the multiplicative action.
    pub fn multiplicative_action_stabilizer_with_limit(
        &self,
        point: usize,
        max_size: u64,
    ) -> PyResult<Vec<Perm>> {
    ...
    /// Uses the default enumeration bound `max_size = 4096`.
    pub fn multiplicative_action_stabilizer_size(&self, point: usize) -> PyResult<usize> {
        self.multiplicative_action_stabilizer_size_with_limit(point, 4096)
    }

    /// Return the size of the stabilizer of `point` under the multiplicative action.
    pub fn multiplicative_action_stabilizer_size_with_limit(
        &self,
        point: usize,
        max_size: u64,
    ) -> PyResult<usize> {
        Sn::new(self.size() as usize)?.stabilizer_size(point, self.multiplicative_group_perms_with_limit(max_size)?)
    }

    /// Return whether the multiplicative action is transitive on all field elements.
    /// Uses the default enumeration bound `max_size = 4096`.
    pub fn is_multiplicative_action_transitive(&self) -> PyResult<bool> {
        self.is_multiplicative_action_transitive_with_limit(4096)
    }

    /// Return whether the multiplicative action is transitive on all field elements.
    pub fn is_multiplicative_action_transitive_with_limit(&self, max_size: u64) -> PyResult<bool> {
        Sn::new(self.size() as usize)?.is_transitive(self.multiplicative_group_perms_with_limit(max_size)?)
    }
...
mod tests {
    ...
    #[test]
    fn fq_multiplicative_action_helpers_match_expected_structure() {
        let k = Fq::new(2, vec![1, 1, 0, 1]).unwrap();
        assert_eq!(k.multiplicative_action_subgroup_size().unwrap(), 7);
        assert_eq!(
            k.multiplicative_action_orbits().unwrap(),
            vec![vec![0], vec![1, 2, 3, 4, 5, 6, 7]]
        );
        assert_eq!(k.multiplicative_action_stabilizer_size(1).unwrap(), 1);
        assert_eq!(k.multiplicative_action_stabilizer_size(0).unwrap(), 7);
        assert!(!k.is_multiplicative_action_transitive().unwrap());
    }
}
