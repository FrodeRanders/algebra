...
impl Fp {
    ...
    pub fn multiplicative_action_subgroup_size(&self) -> PyResult<usize> {
        Sn::new(self.p as usize)?.subgroup_size(self.multiplicative_group_perms()?)
    }
    ...
    pub fn multiplicative_action_orbits(&self) -> PyResult<Vec<Vec<usize>>> {
        Sn::new(self.p as usize)?.orbits(self.multiplicative_group_perms()?)
    }
    ...
    pub fn multiplicative_action_stabilizer_size(&self, point: usize) -> PyResult<usize> {
        Sn::new(self.p as usize)?.stabilizer_size(point, self.multiplicative_group_perms()?)
    }
    ...
    pub fn is_multiplicative_action_transitive(&self) -> PyResult<bool> {
        Sn::new(self.p as usize)?.is_transitive(self.multiplicative_group_perms()?)
    }
...
mod tests {
    ...
    #[test]
    fn fp_multiplicative_action_helpers_match_expected_structure() {
        let f = Fp::new(7).unwrap();
        assert_eq!(f.multiplicative_action_subgroup_size().unwrap(), 6);
        assert_eq!(
            f.multiplicative_action_orbits().unwrap(),
            vec![vec![0], vec![1, 2, 3, 4, 5, 6]]
        );
        assert_eq!(f.multiplicative_action_stabilizer_size(1).unwrap(), 1);
        assert_eq!(f.multiplicative_action_stabilizer_size(0).unwrap(), 6);
        assert!(!f.is_multiplicative_action_transitive().unwrap());
    }
