...
For a narrative guide to arithmetic as permutation actions, see [`docs/finite-actions-guide.md`](docs/finite-actions-guide.md).
...
- The library now exposes matching convenience wrappers for these multiplicative actions, so you can ask for subgroup sizes, orbits, and stabilizers directly from `Fp`, small `Fq`, and `Zn`.
...
- in `Z/12Z`, the stabilizer of `1` under the unit action has size `1`, while the stabilizer of `6` has size `4`
...
```terminaloutput
(.venv) ➜  python play/fp_permutation_actions.py
GF(7)
generator = 3 (mod 7) with multiplicative order 6

subgroup generated by x -> 3x
  size = 6
  orbits = [[0], [1, 2, 3, 4, 5, 6]]
  stabilizer size of 1 = 1

(.venv) ➜  python play/zn_permutation_actions.py
Unit-group action on Z/12Z
modulus = 12
subgroup size = 4
orbits = [[0], [1, 5, 7, 11], [2, 10], [3, 9], [4, 8], [6]]
transitive? = False
stabilizer size of 1 = 1
stabilizer size of 6 = 4
```
